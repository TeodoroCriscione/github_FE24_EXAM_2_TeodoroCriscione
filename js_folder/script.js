...
// ----------- EXTRA ---------------
// Add a Task - This form is a Modal 
document.addEventListener("DOMContentLoaded", () => {
    const addTaskBtn = document.getElementById("add-btn");
    const taskForm = document.getElementById("taskForm");
    
    // Open the modal when clicking "Add Task"
    ...
    taskForm.addEventListener("submit", (event) => {
        ...
        const taskImage = document.getElementById("taskImage").value.trim();
        const taskDescription = document.getElementById("taskDescription").value.trim();
        const taskPriority = parseInt(document.getElementById("taskPriority").value, 10);
        const taskDeadline = document.getElementById("taskDeadline").value;

        if (!taskTitle || !taskImage || !taskDescription || isNaN(taskPriority) || !taskDeadline) {
        ...
        const newTask = {
            ...
            activityName: taskTitle,
            coverImage: taskImage,
            activityDescription: taskDescription,
            priorityLevel: taskPriority,
            activityDeadline: taskDeadline
        };

        // Retrieve existing tasks from Local Storage
        let tasks = JSON.parse(localStorage.getItem("priorityData")) || [];
        
        // Add new task to the list
        tasks.push(newTask);

        // Save updated task list in Local Storage
        localStorage.setItem("priorityData", JSON.stringify(tasks));
